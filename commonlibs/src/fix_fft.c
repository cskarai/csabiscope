...
extern const fixed Loudampl[N_LOUD];
...
int isqrt(int n)
{
	int root = 0, bit, trial;
	bit = (n >= 0x10000) ? (int)1<<30 : (int)1<<14;
	do
	{
		trial = root+bit;
		if (n >= trial)
...
main(){
        ...
        for (i=0; i<N; i++){
                printf("%d: %d, %d -> %d\n", i, real[i], imag[i], isqrt(real[i]*real[i] + imag[i] * imag[i]));
        }
}
#endif  /* MAIN */
